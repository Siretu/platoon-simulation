...
def calc_route(start_loc, goal_loc, verbose=False):
  ...
  if verbose: print "GET", "/viaroute?loc={:.6f},{:.6f}&loc={:.6f},{:.6f}&geometry=false".format(*(start_loc+goal_loc))
  ...
  for i in xrange(real_edge_indices.shape[0]-1):
    compr_link_lengths.append(np.sum(link_lengths[real_edge_indices[i]+1+offset:real_edge_indices[i+1]+1+offset]))
  ...
  processed_route = {'node_ids':node_ids,'link_lengths':compr_link_lengths,'node_coords_lat':node_coords_lat,'node_coords_lon':node_coords_lon}
