...
def get_W_n_I_msp(leader, followers, start_times, arrival_dlines, arrival_times, merge_dists, split_dists, merge_times, split_times, default_speed):
...
def get_W_n_I_msp_simple_call(leader, followers, problem_data, path_data_sets, plans):
  ...
  return get_W_n_I_msp(leader, followers, start_times, arrival_dlines, arrival_times, merge_dists, split_dists, merge_times, split_times, default_speed)
...
def solve(W_n,P_n,T_n,leader, followers, v_min, v_max, start_times, arrival_dlines, arrival_times, I_m, I_sp,F):
  ...
  def F(T=None, z=None):
    ...
    T = np.array(T)
    f = np.sum(np.divide(np.multiply(F1_vec,np.power(W_vec,2)).reshape([1,-1]),T.reshape([1,-1]))) # objective function
    f = co.matrix(f)
    Df = co.matrix(-1*np.divide(np.multiply(F1_vec,np.power(W_vec,2)).reshape([1,-1]),np.power(T,2).reshape([1,-1]))) # gradient transpose
    if z is None: return f, Df
    H = co.spdiag(z[0] * co.matrix(2*np.divide(np.multiply(F1_vec,np.power(W_vec,2)).reshape([1,-1]),np.power(T,3).reshape([1,-1])))) # compute hessian (spdiag: matlab diag)   
    return f, Df, H
...
def optimize_cluster(leader, followers, problem_data, path_data_sets, plans):
  ...
  T_star, obj_opt, obj_init = solve(W_n,P_n,T_n,leader, followers, v_min, v_max, start_times, arrival_dlines, arrival_times, I_m, I_sp,problem_data['F'])
